...
public class Repository {
    ...
    public Customer checkpassword(String lastnameInp, String passwordInp) {
        City city = new City();
        Customer customer = new Customer();
        ResultSet result = null;
        int resultcounter = 0;
        String query =  "SELECT customer.id, customer.firstname, customer.lastname, customer.password, city.id, city.name " +
                        "FROM shoesdb.customer " +
                        "inner join city " +
        ...
        try (Connection con = DriverManager.getConnection(
            ...
            stmt.setString(1, lastnameInp);
            stmt.setString(2, passwordInp);
            result = stmt.executeQuery();

            while (result.next()) {
                city = new City(result.getInt("city.id"),
                        result.getString("city.name"));
                customer = new Customer(result.getInt("customer.id"),
                        result.getString("customer.firstname"),
                        result.getString("customer.lastname"),
                        result.getString("customer.password"));
    ...
    public List<Shoes> getProductsInTheCart(int customerIdInp, int orderIdInp) {
        ...
        try (Connection con = DriverManager.getConnection(
            ...
            while (shoesFound.next()) {
                if (checkOrderedItemId == shoesFound.getInt("IDorderedItem") /*|| checkShoesId == shoesFound.getInt("IDShoes")*/) {
                    Category category = new Category(shoesFound.getInt("categoryID"), shoesFound.getString("Category"));
                    categoryList.add (category);
                }
                else {
